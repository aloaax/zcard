...
import {
    ...
    FileText, Link as LinkIcon, Cloud, CloudOff, Code, Download, Copy, Copy as DuplicateIcon
} from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
    ...
    const handleDuplicate = async (card: BusinessCardData) => {
        if (window.confirm('هل أنت متأكد من تكرار هذه البطاقة؟')) {
            setLoading(true);
            try {
                const newId = Date.now().toString();
                const duplicatedCard: BusinessCardData = {
                    ...card,
                    id: newId,
                    slug: `${card.slug}-copy-${newId.slice(-4)}`,
                    fullName: `${card.fullName} (نسخة)`,
                    fullNameEn: `${card.fullNameEn} (Copy)`,
                };
                await cardService.saveCard(duplicatedCard);
                await loadCards();
                setShowToast(true);
                setTimeout(() => setShowToast(false), 3000);
            } catch (e) {
                alert("فشل التكرار. يرجى التحقق من صلاحيات Firebase (Firestore Rules) أو الاتصال بالإنترنت.");
            } finally {
                setLoading(false);
            }
        }
                            ...
                            <div className="flex flex-col">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`flex items-center gap-3 px-5 py-4 text-sm font-bold transition-all border-r-4 ${activeTab === tab.id
